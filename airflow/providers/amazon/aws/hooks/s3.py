...
class S3Hook(AwsBaseHook):
    ...
    def load_file(
        ...
        get_hook_lineage_collector().add_output_dataset(
            {"scheme": "s3", "bucket_name": bucket_name, "key": key}, self
        )
    ...
    def copy_object(
        ...
        **kwargs,
        ...
        get_hook_lineage_collector().add_input_dataset(
            {"scheme": "s3", "bucket_name": source_bucket_name, "key": source_bucket_key}, self
        )
        get_hook_lineage_collector().add_output_dataset(
            {"scheme": "s3", "bucket_name": dest_bucket_name, "key": dest_bucket_key}, self
        )
        return response
    ...
    def download_file(
        ...
        if preserve_file_name:
            ...
            get_hook_lineage_collector().add_output_dataset({"scheme": "file", "path": ""}, self)
